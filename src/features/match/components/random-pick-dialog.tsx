...
export function RandomPickDialog({ open, onOpenChange, candidates, usernames }: Props) {
  ...
  const canOpenPick = phase === 'landed';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className={cn(
          'w-[min(560px,calc(100vw-3rem))] max-w-none gap-0 rounded-2xl border border-[var(--line-soft)] bg-[var(--bg-1)] p-7',
      ...
      >
        ...
        <DialogClose
          aria-label="Close"
          className="hover:text-foreground focus-visible:ring-ring/50 absolute top-4 right-4 grid place-items-center rounded-lg p-1.5 text-[var(--ink-3)] outline-none focus-visible:ring-3"
        >
          <X className="size-4" />
        </DialogClose>
